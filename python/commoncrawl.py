...
# Metrics
download_retries = Counter('commoncrawl_download_retries_total', 'Number of retried downloads')
download_failures = Counter('commoncrawl_download_failures_total', 'Failed download attempts')
download_successes = Counter('commoncrawl_download_successes_total', 'Successful downloads')
download_time = Histogram('commoncrawl_download_duration_seconds', 'Time spent downloading')
...
class CSVIndexReader(IndexReader):
    ...
    def __enter__(self):
        ...
        try:
            ...
            logger.error(f"Failed to open index file {self.filename}: {e}")
            raise
    ...
    def __iter__(self):
        if not self.reader:
            raise RuntimeError("CSVIndexReader must be used as a context manager")
        return self
    ...
    def __next__(self):
        ...
        try:
            ...
            logger.error(f"Error reading CSV row: {e}")
            raise
